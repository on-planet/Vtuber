...
public class btnLeftJumpHandler implements EventHandler<MouseEvent>{
    private final PlayNextRunnable playNext;
    public btnLeftJumpHandler(PlayNextRunnable playNext){
        this.playNext = playNext;
    }
    @Override
    public void handle(MouseEvent mouseEvent) {
        players.get(currentPlayerIndex).stop();
        if (currentPlayerIndex - 1 >= 0) {   //如果在该索引上存在音乐
            currentPlayerIndex--;
